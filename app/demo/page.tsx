    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .glass-card {
          ...
          border-radius: 24px;
          box-shadow: 0 0 0 1px rgba(255,255,255,0.2), 0 4px 24px rgba(100,80,50,0.06), 0 1px 4px rgba(100,80,50,0.04), inset 0 1px 0 rgba(255,255,255,0.6);
          position: relative;
          overflow: hidden;
        }
        .glass-card::before {
        ...
        .glass-btn-primary {
          ...
          box-shadow: 0 4px 18px rgba(122,158,126,0.3), 0 1px 4px rgba(122,158,126,0.15), inset 0 1px 0 rgba(255,255,255,0.3);
        ...
        .nav-tab-active {
          ...
          border-color: rgba(122,158,126,0.35) !important;
      ...
      <div style={{ minHeight: "100vh", background: "#f4f0ea", position: "relative", overflow: "hidden" }}>
        ...
        <header style={{
          ...
          borderBottom: "1px solid rgba(122,100,70,0.06)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{
              ...
              boxShadow: "0 4px 12px rgba(122,158,126,0.25), inset 0 1px 0 rgba(255,255,255,0.35)"
            }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            <button className={`nav-tab ${page === "chat" ? "nav-tab-active" : ""}`} onClick={() => setPage("chat")}>
              <IconChat size={16} /> Policy Chat
            </button>
            <button className={`nav-tab ${page === "quiz" ? "nav-tab-active" : ""}`} onClick={() => setPage("quiz")}>
              <IconTarget size={16} /> Daily Quiz
            </button>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#b0a08c", fontWeight: 500 }}>
            <IconGlobe size={13} />
            MA Home Care Regulations
          </div>
        </header>
        ...
        <div style={{
          ...
          borderTop: "1px solid rgba(122,100,70,0.08)",
          display: "flex", alignItems: "center", justifyContent: "center", gap: "16px",
          zIndex: 20,
        }}>
          <span style={{ fontSize: "13px", color: "#5a5040", fontWeight: 500 }}>
            This demo uses general MA regulations. Want to see it loaded with your agency's handbook?
          </span>
          <a
            href="mailto:thomasjacekim@gmail.com?subject=Interested in PolicyPulse for my agency"
            style={{
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ question: msg, organization_id: ORG_ID }),
      });
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "20px" }}>
        {SUGGESTED_QUESTIONS.map((sq, i) => (
          <button key={i} className="suggest-chip" onClick={() => sendMessage(sq.text)}>
            {sq.lang === "es" && <span style={{ marginRight: "4px" }}>🇪🇸</span>}
            {sq.text}
          </button>
        ))}
      </div>
      ...
      <div className="glass-card" style={{ display: "flex", flexDirection: "column", height: "520px" }}>
        <div style={{ flex: 1, overflow: "auto", padding: "24px 22px", display: "flex", flexDirection: "column", gap: "16px" }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", gap: "12px", alignItems: "flex-start", flexDirection: m.role === "user" ? "row-reverse" : "row" }}>
              <div style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                ...(m.role === "ai"
                  ? { background: "linear-gradient(135deg, rgba(122,158,126,0.85) 0%, rgba(95,135,100,0.95) 100%)", boxShadow: "0 2px 8px rgba(122,158,126,0.25)" }
                  : { background: "rgba(255,255,255,0.6)", border: "1px solid rgba(255,255,255,0.8)", color: "#8a7a65", fontWeight: 600, fontSize: "11px" }
                )
              }}>
                {m.role === "ai" ? <IconLayers size={15} color="#fff" /> : "You"}
              </div>
              <div style={{ maxWidth: "72%" }}>
                <div className={m.role === "ai" ? "chat-bubble-ai" : "chat-bubble-user"} style={{ padding: "14px 18px", fontSize: "14px", lineHeight: 1.7 }}>
                  {m.text}
                </div>
                {m.citation && (
                  <div style={{
                    ...
                    borderRadius: "8px", padding: "4px 12px", fontSize: "11px", color: "#2c5a30", fontWeight: 600
                  }}>
          ...
          {loading && (
            <div style={{ display: "flex", gap: "12px", alignItems: "flex-start" }}>
              <div style={{ width: "34px", height: "34px", borderRadius: "10px", background: "linear-gradient(135deg, rgba(122,158,126,0.85) 0%, rgba(95,135,100,0.95) 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid rgba(122,100,70,0.06)", display: "flex", gap: "10px" }}>
          <input
            ...
            placeholder="Ask a compliance question in English or Spanish..."
            style={{ flex: 1 }}
          />
          <button className="glass-btn-primary" onClick={() => sendMessage()} disabled={loading || !input.trim()} style={{ width: "48px", height: "48px", padding: 0 }}>
            <IconSend size={17} />
          </button>
        </div>
      </div>
...
function QuizDemo() {
  ...
  function getOptionText(q: QuizQuestion, opt: typeof OPTIONS[number]) {
    return q[`option_${opt}` as keyof QuizQuestion] as string;
  }

  function correctIdx(q: QuizQuestion) {
    return OPTIONS.indexOf(q.correct_option.toLowerCase() as typeof OPTIONS[number]);
  }
  ...
  function pickAnswer(idx: number) {
    ...
    setSelected(idx);
    setShowResult(true);
    const correct = idx === correctIdx(current);
    setTimeout(() => {
      const newAnswers = [...answers, { correct, selected: idx }];
          ...
          <div className="glass-card" style={{ padding: "32px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {OPTIONS.map((opt, i) => {
                ...
                return (
                  <button key={opt} className="quiz-option-btn" onClick={() => pickAnswer(i)} disabled={showResult} style={extraStyle}>
                    <span style={{ fontWeight: 700, color: showResult && isCorrect ? "#2c5a30" : "#7a9e7e", marginRight: "10px", fontSize: "13px", flexShrink: 0 }}>{opt.toUpperCase()}</span>
                    <span style={{ flex: 1 }}>{getOptionText(current, opt)}</span>
            ...
            {showResult && (
              <div style={{ marginTop: "20px", padding: "14px 18px", borderRadius: "14px", background: selected === correctIdx(current) ? "rgba(90,138,94,0.06)" : "rgba(184,92,82,0.05)", border: `1px solid ${selected === correctIdx(current) ? "rgba(90,138,94,0.18)" : "rgba(184,92,82,0.15)"}`, fontSize: "13px", color: "#5a5040", lineHeight: 1.6 }}>
                {current.explanation}
              </div>
            )}
          </div>
